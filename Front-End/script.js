...
var source = new EventSource('NewServlet');
var response;

function f(p){
    var pending = p.data;
    //console.log(response);
    response = JSON.parse(pending);
    //console.log(response);
}
...
$(document).keydown(function (e) {
	...
	console.log(key);
	sendPlayerPosition();
        //document.getElementById("keypress").submit();
});         
...
function sendPlayerPosition(){    
	if(true){
		...
		xmlhttprequest.open("POST","NewServlet?keypress=" + keypress,true);
		xmlhttprequest.send();
	}else{ 
		return;
	} 
